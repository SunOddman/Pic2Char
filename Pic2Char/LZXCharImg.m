...
- (NSImage *)charsImageWithImage:(NSImage *)imgOriginal {
    ...
    // 非动图
    if (!arrFrames || arrFrames.count == 1) {
        return [self charImgBySingleBMP:imgOriginal];
    }
    ...
    for (NSImage *imgF in arrFrames) {
        NSImage *imgR = [self charImgBySingleBMP:imgF];
        
        [imgsArr addObject:imgR];
    }
    
    NSData *data = [imgsArr dataOfGifFramesWithDuration:imgOriginal.duration];
    imgReturn = [[NSImage alloc] initWithData:data];
    
    return imgReturn;
}

- (NSImage *)charImgBySingleBMP:(NSImage *)imgF {
    NSImage *imgReturn;
